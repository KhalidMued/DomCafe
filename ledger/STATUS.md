...
- Frontend suite: `83 passed`; coverage includes all three live surfaces (admin orders, dedicated guest status, and menu progress), reconnect catch-up, notification refetches, 15-second unsupported/disconnected/error fallback, 60-second connected reconciliation, terminal/404/logout/unmount cleanup, stale-read rejection, and status-write reconciliation.
- Frontend production build and `docker compose config -q` pass. The Docker stack was rebuilt and all services are healthy; Nginx syntax, `/api/health`, and PgBouncer checks pass. Through the live Nginx edge, an unauthenticated admin stream returns `401`, an unknown guest code returns `404`, and an existing private guest stream received `connected` followed by a Redis-published `order-changed`, with only `data: {}` payloads. No order data was changed.
- Independent review found and resolved two issues before deployment: connected streams now perform bounded 60-second reconciliation after a swallowed Redis publication failure, and Nginx now rate-limits stream connection attempts in addition to limiting concurrent streams.
- `npm audit --audit-level=high` still reports pre-existing development-tooling advisories in the pinned Vite/Vitest dependency tree; this branch adds no dependency and does not expand that scope.

Verification for `feat/admin-order-submitted-time` (2026-09-08):
